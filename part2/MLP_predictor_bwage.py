...
# tasks.remove('dailyconfirmed')
# tasks.remove('dailydeceased')
tasks.remove('dailyrecovered')
tasks.remove('date')
tasks.remove('totalconfirmed')
tasks.remove('totaldeceased')
tasks.remove('totalrecovered')
network_list = [
    [10, 100, 32, 10, 1],
    [10, 100, 100, 1],
    [10, 100, 32, 100, 1],
...
for task in tasks:
    raw_train_data = [0.23 * x for x in data_loader[task]]
    ...
    for i in range(len(network_list)):
        ...
        for itr in range(EPOCHS):
            if itr % 100 == 0:
                ...
                file_path = f'./graphs/{task + "_bwage"}/{i+1}/MLP_predction{itr}.png'
                predict(train_data, line=line, title=title, file_path=file_path, future=15)
